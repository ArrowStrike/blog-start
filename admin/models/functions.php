...
// Запрос
    if ($image == null) {
        $templateUpdate = "UPDATE articles SET category_id='%d', title='%s', text='%s' WHERE id='%d'";
...
//     mysqli_real_escape_string($link, $pubdate),
        ...
        $templateUpdate = "UPDATE articles SET category_id='%d', title='%s',image='%s', text='%s' WHERE id='%d'";
        $query = sprintf($templateUpdate,
            $category_id,
            mysqli_real_escape_string($link, $title),//проводит экранцию входящих параметров, добавляет обратный слэш, перед символями, которые могут испортить sql запрос
            mysqli_real_escape_string($link, $image),
...
// Пути загрузки файлов
    $path = '../static/images/';
    $pathResize = '../static/imagesPreview/';
    $tmpPath = '';
    $tmpPathPreview = '../';
...
function deleteImage($link, $articleID)
{
    ...
    if ($totalCount['repeats'] == 1) { //проверка на повторы
        $imageDeletePath = '../static/images/' . $article['image'];
        $imagePreviewDeletePath = '../static/imagesPreview/' . $article['image'];
